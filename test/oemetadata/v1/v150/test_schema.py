...
def test_jsonschema_should_validate_oemetadata_schema():
    ...
    try:
        validate(OEMETADATA_V150_SCHEMA, OEMETADATA_JSONSCHEMA_DRAFT07_SCHEMA)
        print("OEMetadata Schema (v1.5.0) is valid JSON Schema.")
    except ValidationError as e:
        print("Cannot validate OEMetadata Schema with JSON Schema (v1.5.0)!", e)
...
def test_oemetadata_schema_should_have_correct_path():
    ...
    assert get_string(OEMETADATA_V150_SCHEMA["$schema"]) == get_string(
        "http://json-schema.org/draft-07/schema#"
    ), "Wrong schema path in OEMetadata Schema (v1.5.0)!"

    assert get_string(OEMETADATA_V150_SCHEMA["$id"]) == get_string(
        "https://raw.githubusercontent.com/OpenEnergyPlatform/oemetadata/production/oemetadata/v1/v150/schema.json"
    ), "Wrong id path in OEMetadata Schema (v1.5.0)!"
