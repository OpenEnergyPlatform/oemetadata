...
def test_oemetadata_schema_should_load():
    try:
        ...
        print("Cannot open OEMetadata Schema (v2.1)!")
...
def test_jsonschema_should_validate_oemetadata_schema():
    ...
    try:
        validate(OEMETADATA_V21_SCHEMA, OEMETADATA_JSONSCHEMA_DRAFT202012_SCHEMA)
        print("OEMetadata Schema (v2.1) is valid JSON Schema (Draft 2020-12).")
    except ValidationError as e:
        print("Cannot validate OEMetadata Schema with JSON Schema (v2.1)!", e)
...
def test_oemetadata_schema_should_have_correct_path():
    ...
    assert get_string(OEMETADATA_V21_SCHEMA["$schema"]) == get_string(
        "https://json-schema.org/draft/2020-12/schema"
    ), "Wrong schema path in OEMetadata Schema (v2.1)!"

    assert get_string(OEMETADATA_V21_SCHEMA["$id"]) == get_string(
